...
//         const {
//             name, description, category, sku, unit, price, 
//             stock, discountAmount
//         } = req.body;
...
export const getProductById = async (req, res) => {
    try {
        ...
        // Fetch from database and populate details
        const product = await populateProductDetails(Product.findById(productId));
...
export const addReview = async (req, res) => {
    try {
        ...
        await product.save();
        sendSuccessResponse(res, product, "Review added successfully");
    } catch (error) {
        sendErrorResponse(res, error);
    }
};
