...
export const productValidationSchema = Joi.object({
    ...
    discountAmount: Joi.number().optional().positive().messages({
        'number.base': 'Discount amount should be a number',
        'number.positive': 'Discount amount should be greater than 0',
    }),
    ...
    thumbnail: Joi.string().optional().messages({
        'string.base': 'Thumbnail should be a string',
        'string.empty': 'Thumbnail cannot be empty',
    }),
    
    images: Joi.array().items(Joi.string().messages({
        'string.base': 'Image URLs should be strings',
    })).optional(),

    status: Joi.string().valid('pending', 'approved', 'rejected').optional().messages({
        'string.base': 'Status should be a string',
        'any.only': 'Status should be one of "pending", "approved", or "rejected"',
    }),
});
